...
with col1:
    ...
    st.write("Models that learn from labeled data (Input $\to$ Output).")
    

    st.page_link("pages/1_Linear_Regression.py", label="Start Linear Regression", icon="📏")
    st.page_link("pages/2_Logistic_Regression.py", label="Start Logistic Regression", icon="🧬")
    st.page_link("pages/3_KNN.py", label="K-Nearest Neighbors (KNN)", icon="📍")

with col2:
    st.warning("### 2. Unsupervised Learning")
    st.write("Models that find hidden patterns without labels.")
    
    # Day 4: K-Means (Coming Soon)
